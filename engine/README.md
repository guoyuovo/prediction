...
### 赛果驱动·实时校准（calibrate-live.mjs）
不拟合任何外部文章，只借鉴"概率校准"的思路：用 `data/backtest-v2.json` 累积的**世界杯真实赛果**（样本外预测 vs 实际比分），网格搜索一个**平局概率乘子 `draw.liveMult`**，最小化 logloss——直击早期"平局预测偏少"的问题。

- **三道防过拟合闸**：样本 <20 场跳过不改配置；按样本量收缩最优值（n 越大越敢动）；绝对乘子夹在 [0.5, 2.0]
- 在 daily 流水线"滚动 xG → 校准 → 重算预测"之间自动执行；乘子写入 `config/model.json` 的 `draw.liveMult`（默认 1 = 不改动）
- 试跑：`node scripts/calibrate-live.mjs --dry-run`；阈值可用环境变量 `CALIB_MIN` 覆盖
